...
static void gml_e_inlne_common( const gmltag * entry, e_tags t )
{
    ...
    if( nest_cb->c_tag != t ) {         // unexpected exxx tag
        ...
        if( script_style_sav.style != SCT_none ) {
            if( t != t_Q ) {
                scr_style_copy( &script_style_sav, &script_style );
...
void gml_sf( const gmltag * entry )
{
        ...
        for( ;; ) {
            ...
            if( !strnicmp( "font", p, 4 ) ) {
                ...
                if( g_att_val.val_start != NULL ) {
                    font = get_font_number( g_att_val.val_start, g_att_val.val_len );
                    font_seen = true;
                }
    ...
    } else {                          // font is a required attribute
        xx_line_err_c( err_att_missing, g_att_val.val_start );
    }
